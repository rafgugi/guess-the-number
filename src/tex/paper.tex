...
\begin{proof}
Pembuktian $0 \le wt(\vec{c}) \le e$ dapat dibuktikan dengan dua kasus.

1) Jika $wt(\vec{c}) = 0$ maka
\begin{align*}
d_H(\vec{c},\vec{x_i}) &\ge d \mid \vec{c} \neq \vec{x_i} , \vec{x_i} \in L' \\
wt(\vec{c} + \vec{x_i}) &\ge d \\
wt(\vec{x_i}) &\ge d \label{eq:proofd} \stepcounter{equation} \tag{\theequation}
\end{align*}
...
d_H(\vec{c},\vec{x_i}) &\ge d \mid \vec{c} \neq \vec{x_i} , \vec{x_i} \in L' \\
wt(\vec{c}+\vec{x_i}) &\ge d \\
wt(\vec{c}) + wt(\vec{x_i}) \ge wt(\vec{c}+\vec{x_i}) &\ge d \\
wt(\vec{c}) + wt(\vec{x_i}) &\ge d \\
e + wt(\vec{x_i}) &\ge d \\
wt(\vec{x_i}) &\ge 2e+1-e \\
wt(\vec{x_i}) &\ge e+1\\
wt(\vec{x_i}) &> e
\end{align*}
Jadi jika ada codeword $c$ dimana $0 \le wt(c) \le e$ maka $wt(\vec{x_i}) > e$ dimana $\vec{x_i} \neq c$.
\end{proof}
