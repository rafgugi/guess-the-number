...
Pada algoritma \textit{greedy} ini, fungsi seleksi untuk memilih kandidat query terbaik pada setiap iterasi ada empat macam, yang nanti akan diuji untuk dipilih fungsi seleksi terbaik. Fungsi seleksi tersebut adalah sebagai berikut:
\begin{itemize}
  \item Jarak minimum terbesar;
  \item Jarak minimum terbesar diikuti dengan jarak maximum terkecil;
  \item Yang paling kecil dari jarak maximum dikurangi dengan jarak minimum;
  \item Jarak minimum terbesar diikuti dengan jumlah angka yang memiliki nilai minimum terkecil.
\end{itemize}
Keempat fungsi seleksi tersebut ditentukan karena \textit{global optimum} yang dicari adalah jarak minimum terbesar, selisih nilai jarak maximum dan jarak minimum harus sekecil mungkin, dan mencari query yang dapat menghabiskan sebanyak mungkin angka dengan jarak minimum.
...
\KwResult{$code\_order$}
  ...
  \While{$code\_min < d$}{\label{alg:best_order_loop}
    ...
    \For{$i = 1$ \KwTo $M$}{\label{alg:best_search_loop}
      ...
      \If{meet the greedy selection function}{\label{alg:greedy_selection}
        $best = i$\;
