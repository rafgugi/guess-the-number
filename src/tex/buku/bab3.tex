...
\KwResult{$queries$}
  ...
  $minimal[d] = [\ ]$\;
  $distances[M] = [\ ]$\;
  $code\_order$ = preloaded exhaustive\_search($2 \leq 2^m \leq 4096$, $16$)\;\label{alg:preloaded}
  ...
  $qrep$ = $d/(M/2)$\;\label{alg:qrep}
  $dmod$ = $d$ mod $(M/2)$\;\label{alg:dmod}
  $solution = [\ ]$\;

  \For{$i = 0$ \KwTo $qrep$}{
    \For{$j = 0$ \KwTo $M-1$}{
      $solution$.push($queries[j]$)\;
    }
  }
  \For{$i = 0$ \KwTo $minimal[dmod]$}{
